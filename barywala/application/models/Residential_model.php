<?php
	...
	{     
        ...
        //Add data to residential table 
        public function add_residential($image , $image1, $image2, $image3, $image4) {
            ...
            $data = array(
                ...
                'bachelor' => $this->input->post('bachelor'),
            ...
            return $this->db->insert('residential',$data);
        }

        //View all residential data with pagination
        public function view_all_residential($slug = false, $limit = false, $offset = false ){ 
            if($limit){
                $this->db->limit($limit,$offset );
            } 
            
            if($slug == false){
                $this->db->order_by('residential.id','DESC');//show the post in LIFO
                $query = $this->db->get('residential');
                return $query->result_array();
            }

            $query = $this->db->get_where('residential',array('slug' => $slug));
            return $query->row_array();
        } 

        //View all residential data without pagination
         public function all_residential_list($slug = false, $limit = false, $offset = false){
              if($limit)
            {
                $this->db->limit($limit,$offset );
            }  
            if($slug == false){
                $this->db->order_by('residential.id','DESC');
                $query = $this->db->get('residential');
                return $query->result_array();
            }
        ...
        public function get_post($search)
        {
           ...
           if($search != ''){
            ...
            $this->db->or_like('veranda', $search);
            $this->db->or_like('parking', $search);
            $this->db->or_like('gas', $search);
            $this->db->or_like('bills', $search);
            $this->db->or_like('smoking', $search);
            $this->db->or_like('bachelor', $search);
            }
            $query = $this->db->get();
            return $query->result_array();
        }
        ...
        //get data posted by user id
         public function view_all_residential_for_user($user_id){       
            $user_id = $this->session->userdata('id');
            $this->db->order_by('residential.id', 'DESC');
            $query = $this->db->get_where('residential', array('user_id' => $user_id));
            return $query->result_array();
        }  


        //Update value of residential table
        public function update_residential_post()
        {
            $slug = url_title($this->input->post('title'));
             $data = array(
                'title' => $this->input->post('title'),
             ...
             return $this->db->update('residential',$data);
        }


        //Delete data from residential table
        public function delete_post($id){
        	$this->db->where('id',$id); 
        	$this->db->delete('residential');
        	return true;
        }
        ...
        public function check_title_exists($title){
            $query = $this->db->get_where('residential', array('title' => $title));
